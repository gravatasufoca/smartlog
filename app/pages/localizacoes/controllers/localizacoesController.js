define(['msAppJs',
    ...
    app.controller('localizacoesController', ['$scope',
        ...
        function ($scope,
            ...
            $scope.abreDeslocamento = function () {
                msModalService.setConfig({
                    backdrop: true,
                    keyboard: false,
                    modalFade: true,
                    template: null,
                    templateUrl: './app/pages/localizacoes/directives/templates/modalDeslocamento.html',
                    controller: ['$scope',
                        '$rootScope',
                        '$modalInstance',
                        '$msNotifyService',
                        ...
                        function ($scope,
                            ...
                            NgMap.getMap("mapa").then(function (map) {
                                ...
                                var usuario = $rootScope.usuarioAutenticado;
                                 if (usuario != null && usuario.perfil != null) {
                                    ...
                                    localizacoesService.recuperarTodasLocalizacoes(usuario.perfil.id).then(function (resposta) {
                                        if(resposta.resultado!=null) {
                                            angular.forEach(resposta.resultado, function (a) {
                                                a = fixLocalizacao(a);
                                                var posicao = new google.maps.LatLng(a.latitude, a.longitude);
                                                a.posicao = posicao;
                                                posicao.data=a.data;
                                                posicao.id=a.id;
                                                $scope.todasLocalizacoes.push(posicao);

                                                var marker = new google.maps.Marker({
                                                    position: posicao,
                                                    map: $scope.map,
                                                    draggable: false,
                                                    title:a.data.format('DD/MM/YYYY HH:mm:ss'),
                                            ...
                                            if (resposta.resultado.length > 0){
                                                $scope.map.setCenter($scope.todasLocalizacoes[$scope.todasLocalizacoes.length-1]);
                                            }
                                        }
                            ...
                            $scope.toogleAnimation = function (localizacao) {
                                _.map($scope.map.markers, function (marker) {
                                    if(marker.position.id==localizacao.id){
                ...
                msModalService.modalInstance.result.then(function (map) {
                    if (map != null) {
