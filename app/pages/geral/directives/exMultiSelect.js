define(['msAppJs'], function(app) {
	//DOCUMENTACAO: http://isteven.github.io/angular-multi-select/
	//VERSAO: 2.0.2

	app.directive( 'exMultiSelect' , [ '$sce', '$timeout', function ( $sce, $timeout ) {
		return {
			restrict: 
				'AE',
				...
				replace: 
					...
					template: 
						...
						'<button type="button" ng-disabled="isDisabled" ng-class="{\'disableButton\' : isDisabled}" class="button multiSelectButton" ng-click="toggleCheckboxes( $event ); refreshSelectedItems(); refreshButton();" ng-bind-html="varButtonLabel">' +
						'</button>' +                              
						...
						'<button type="button" ng-click="select( \'all\',   $event );"    class="helperButton" ng-if="!isDisabled && displayHelper( \'all\' )">   &#10003;&nbsp; {{allLabel ? allLabel : \'Select All\'}}</button> ' +
						'<button type="button" ng-click="select( \'none\',  $event );"   class="helperButton" ng-if="!isDisabled && displayHelper( \'none\' )">  &times;&nbsp; {{noneLabel ? noneLabel : \'Select None\'}}</button>' +
						'<button type="button" ng-click="select( \'reset\', $event );"  class="helperButton" ng-if="!isDisabled && displayHelper( \'reset\' )" style="float:right">&#8630;&nbsp; {{resetLabel ? resetLabel : \'Reset\'}}</button>' +
						'</div>' +
						'<div class="line" style="position:relative" ng-if="displayHelper( \'filter\' )">' +
						'<input placeholder="{{filterLabel ? filterLabel : \'Search...\'}}" type="text" ng-click="select( \'filter\', $event )" ng-model="inputLabel.labelFilter" ng-change="updateFilter();$scope.getFormElements();" class="inputFilter" />' +
						...
						link: function ( $scope, element, attrs ) {           
							...
							$scope.refreshButton = function() {
								...
								if ( $scope.selectedItems.length === 0 ) {
									...
									$scope.varButtonLabel = '<div class="buttonLabel">'+(( typeof $scope.defaultLabel !== 'undefined' ) ? $scope.defaultLabel : 'None selected')+'</div>';
								}
								else {  
									var lastIndexComma = 0;
									...
									//remove last comma
									lastIndexComma = $scope.varButtonLabel.lastIndexOf(', ');
									$scope.varButtonLabel = $scope.varButtonLabel.substring(0, lastIndexComma);							

									if ( $scope.more === true ) {
										// https://github.com/isteven/angular-multi-select/pull/16
										if (tempMaxLabels > 0) {
											$scope.varButtonLabel += ', ... <div class="buttonLabel">(Total: ' + $scope.selectedItems.length + ') </div>';                        
										}
									}
